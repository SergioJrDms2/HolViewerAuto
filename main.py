...
def _groq_key() -> str:
    try: return "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"
...
<div style="background:#fff;border:1px solid #e5e7eb;border-radius:.75rem;
  ...
  <div style="background:{insight_bg};border-left:3px solid {insight_cor};
    ...
    with tab_intel:
        ...
        top_conc     = next((i for i in inst_list if i["tipo"] == "concorrente"), None)
        regime_top   = regimes.most_common(1)[0] if regimes else None

        insights = []

        # Insight 1 — Posicionamento de market share
        if pct_nosso < 20 and total_conc_val > 0:
            insights.append({
                "icone":"🚨","cor":"#fef2f2","borda":"#dc2626",
                "titulo":"Mercado Dominado pela Concorrência",
                "texto":(f"Apenas {pct_nosso:.1f}% do volume consignado mensal está conosco ({fmt(total_nosso_val)}). "
                        f"A concorrência ocupa {pct_conc:.1f}% ({fmt(total_conc_val)}/mês) com {n_conc_ct} contratos ativos. "
                        f"Ação imediata: campanha de Compra de Dívida direcionada aos {len(alvo_puro)} servidores "
                        f"que usam só concorrentes.")
            })
        elif pct_nosso > 50:
            insights.append({
                "icone":"🏆","cor":"#f0fdf4","borda":"#16a34a",
                "titulo":"Liderança de Mercado — Defender e Expandir",
                "texto":(f"Liderança consolidada com {pct_nosso:.1f}% do volume ({fmt(total_nosso_val)}/mês) "
                        f"e {n_nossos_ct} contratos. "
                        f"Estratégia: reter base via refinanciamento e avançar nos {len(sem_consig)} servidores "
                        f"ainda sem consignações — {fmt(margem_disp_total)} de margem disponível no grupo.")
            })
        else:
            insights.append({
                "icone":"⚖️","cor":"#eff6ff","borda":"#1d4ed8",
                "titulo":"Mercado Disputado — Janela de Crescimento",
                "texto":(f"Participação de {pct_nosso:.1f}% ({fmt(total_nosso_val)}/mês) indica espaço real para crescer. "
                        f"Com {len(alvo_puro)} servidores usando só concorrentes e {fmt(margem_disp_total)} de margem "
                        f"de empréstimo disponível no grupo, a Compra de Dívida combinada com Empréstimo "
                        f"é o vetor principal de expansão.")
            })

        # Insight 2 — Mercado virgem
        if len(sem_consig) > 0:
            pct_v = len(sem_consig) / total * 100
            insights.append({
                "icone":"🌱","cor":"#f0fdf4","borda":"#059669",
                "titulo":f"Mercado Novo: {len(sem_consig)} Servidores Sem Consignações ({pct_v:.0f}%)",
                "texto":(f"{pct_v:.0f}% dos servidores nunca contrataram crédito consignado — "
                        f"terreno fértil sem resistência de concorrência. "
                        f"Produto de entrada ideal: Cartão Benefício (zero comprometimento de longo prazo) "
                        f"ou Empréstimo de valor menor para criar primeiro relacionamento. "
                        f"Margem total disponível no grupo: {fmt(margem_disp_total)}.")
            })

        # Insight 3 — Principal concorrente
        if top_conc:
            insights.append({
                "icone":"🎯","cor":"#dbeafe","borda":"#1d4ed8",
                "titulo":f"Principal Alvo Competitivo: {top_conc['desc']}",
                "texto":(f"{top_conc['contratos']} contrato(s) em {top_conc['servidores']} servidor(es) — "
                        f"{fmt(top_conc['valor'])}/mês saindo da folha para essa instituição. "
                        f"Ticket médio de {fmt(top_conc['ticket_medio'])} por contrato. "
                        f"Cada Compra de Dívida convertida aqui elimina essa parcela da folha do cliente "
                        f"e pode gerar troco — argumento de fechamento direto.")
            })

        # Insight 4 — Comparação de tickets
        if ticket_nosso > 0 and ticket_conc > 0:
            diff = ticket_conc - ticket_nosso
            if diff > 50:
                insights.append({
                    "icone":"📊","cor":"#fffbeb","borda":"#d97706",
                    "titulo":"Concorrentes Têm Parcelas Maiores — Oportunidade de Troco",
                    "texto":(f"Nosso ticket médio: {fmt(ticket_nosso)}/mês. "
                            f"Ticket médio concorrentes: {fmt(ticket_conc)}/mês. "
                            f"Diferença de {fmt(diff)}: contratos concorrentes são de maior valor, "
                            f"reforçando que a Compra de Dívida tende a liberar troco real para o cliente. "
                            f"Use esse dado como argumento de abordagem.")
                })
            elif ticket_nosso > ticket_conc + 50:
                insights.append({
                    "icone":"📊","cor":"#f5f3ff","borda":"#7c3aed",
                    "titulo":"Nossos Contratos São de Maior Valor — Sinal de Confiança",
                    "texto":(f"Ticket médio nosso ({fmt(ticket_nosso)}) supera concorrentes ({fmt(ticket_conc)}). "
                            f"Clientes confiam em contratos maiores conosco — explorar refinanciamento "
                            f"e cross-sell de produtos complementares.")
                })

        # Insight 5 — Regime predominante
        if regime_top and regime_top[1] / total > 0.65:
            pct_reg = regime_top[1] / total * 100
            eh_ef   = any(x in regime_top[0].upper() for x in ["ESTATUTÁRIO","EFETIVO","CELETISTA"])
            insights.append({
                "icone":"🏛️","cor":"#f5f3ff","borda":"#7c3aed",
                "titulo":f"Base Homogênea: {pct_reg:.0f}% {regime_top[0]}",
                "texto":(f"Base majoritariamente de regime {regime_top[0]} facilita padronização da abordagem comercial. "
                        + ("Portfolio completo disponível — prazos longos (até 120 meses) e todas as margens aplicáveis."
                            if eh_ef else
                            "Regime limita o portfolio — foco em Auxílio Servidor e Vale Consignado. "
                            "Destaque o diferencial: somos um dos poucos que atendem esse perfil."))
            })

        # Insight 6 — Concentração competitiva
        conc_inst_unicas = len([i for i in inst_list if i["tipo"] == "concorrente"])
        if conc_inst_unicas >= 3:
            insights.append({
                "icone":"🗺️","cor":"#fef3c7","borda":"#d97706",
                "titulo":f"Mercado Fragmentado: {conc_inst_unicas} Concorrentes Diferentes",
                "texto":(f"{conc_inst_unicas} instituições concorrentes identificadas na folha — "
                        f"mercado pulverizado indica que clientes têm múltiplos relacionamentos. "
                        f"Oportunidade de consolidação: Compra de Dívida múltipla (mais de um contrato "
                        f"concorrente por servidor) com maior troco e parcela única conosco.")
            })

        for ins in insights:
            st.markdown(f"""
    <div style="background:{ins['cor']};border-left:4px solid {ins['borda']};
    ...
    <div style="display:flex;align-items:flex-start;gap:.65rem;">
        <span style="font-size:1.1rem;flex-shrink:0;margin-top:.05rem;">{ins['icone']}</span>
        <div>
